			...
			(msg_rd->target_system==get_CommulinkSysId() && msg_rd->target_component==MAV_COMP_ID_CAMERA))
	{
		...
		{ 		
			...
			else
			{
				...
				SendMsgToMode( mode_msg, 0.01 );double vv[3];
				vv[0] = mode_msg.params[4];
				vv[1] = mode_msg.params[5];
				vv[2] = mode_msg.params[6];
				SDLog_Msg_DebugVect("int",vv,3);
			}
		}
	}
}
...
static uint32_t dao_key = 0;	static int8_t dao_sensor = -1;
...
//																							);
			...
			{	//注册成功
				...
				if( need_dao )
				{
					if( dao_key == 0 )
					{
						dao_sensor = msg_rd->ind;
						dao_key = DAOSensorRegister( 1, "SLAM", vector3<double>(100,0,0), true, 0.05 );
					}
				}
			}
		}
		...
		{	//取消注册传感器
			...
			if(res)
			{
				...
				if( dao_sensor==msg_rd->ind && dao_key )
				{
					if( DAOSensorUnRegister( 1, dao_key ) )
					{
						dao_key = 0;
						dao_sensor = -1;
					}
			...
			(msg_rd->target_system==get_CommulinkSysId() && msg_rd->target_component==0) )
	{
		...
		{	//传感器已注册
			...
			if( dao_sensor==msg_rd->ind && dao_key )
			{
				...
				fast_sin_cos( quat.getYaw(), &sin_Yaw, &cos_Yaw );
				
				vector3<double> relPos( 
					BodyHeading2ENU_x( 100, 0, sin_Yaw, cos_Yaw ),
					BodyHeading2ENU_y( 100, 0, sin_Yaw, cos_Yaw ),
					0 );
				DAOSensorUpdate( 1,dao_key, relPos, available );
			}
...
static void Msg144_FOLLOW_TARGET( uint8_t Port_index , const mavlink_message_t* msg )
{
	...
	mode_msg.cmd = 144;
	mode_msg.params[0] = 0;
	mode_msg.params[1] = msg_rd->lat*1e-7;
	mode_msg.params[2] = msg_rd->lon*1e-7;
	mode_msg.params[3] = msg_rd->alt*1e+2;
	mode_msg.params[4] = msg_rd->vel[1]*1e+2;
	mode_msg.params[5] = msg_rd->vel[0]*1e+2;
	mode_msg.params[6] = -msg_rd->vel[2]*1e+2;
	SendMsgToMode( mode_msg, 0.01 );
}
